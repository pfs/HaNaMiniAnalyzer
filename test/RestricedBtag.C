...
#include <iostream>
using namespace std;
const TString massVals[8] = {"20","25","30","35","40","45","55","60"};
...
std::vector<TH1D*> compareSignificances(TString fname, TString higgsType /* "b" or ""*/, TString SigType, TString matchTyp = ""){
	...
	for(int iMass = 0; iMass < 8; iMass++){
		f = TFile::Open(fname+"_"+massVals[iMass]+".root");
...
void RestricedBtag(){
	...
	std::vector<TH1D*> vec3 = compareSignificances("out_mH_ttdy_significance_Reg_WithCuts_all","","LnSoSqrtSB",match);
	std::vector<TH1D*> vec4 = compareSignificances("out_mH_ttdy_significance_Reg_WithCuts_all","","LnSoSqrtSBdB",match);	
