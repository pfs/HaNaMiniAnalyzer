...
bool utils::isParameterAtBoundary( const RooRealVar &param ){
    ...
    fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tErrHi: %f", param.getErrorHi());
    fprintf(CloseCoutSentry::trueStdOutGlobal(), ",\tMax: %f \n",param.getMax());
    fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tPullMax: %f \n",fabs((param.getMax()-param.getVal())/param.getErrorHi()));
    fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tErrLo: %f", param.getErrorLo());
    fprintf(CloseCoutSentry::trueStdOutGlobal(), ",\tMin: %f \n",param.getMin());
    fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tPullMin: %f \n",fabs((param.getVal()-param.getMin())/(-1.0*param.getErrorHi())));
...
bool utils::anyParameterAtBoundaries( const RooArgSet &params, int verbosity ){
	...
	cout << "My parameter: "<<a->GetName()<<endl;
