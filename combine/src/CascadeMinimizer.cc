...
bool CascadeMinimizer::minimize(int verbose, bool cascade) 
{
    ...
    RooLinkedListIter iter = nllParams->iterator(); int i = 0;
    for (RooRealVar *a = (RooRealVar *) iter.Next(); a != 0; a = (RooRealVar *) iter.Next(), ++i) {
	/*fprintf(CloseCoutSentry::trueStdOutGlobal(), "%s -------------------\n", a->GetName());
	fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tValue: %f \n", a->getVal());
	fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tErrHi: %f", a->getErrorHi());
        fprintf(CloseCoutSentry::trueStdOutGlobal(), ",\tMax: %f \n",a->getMax());
        fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tPullMax: %f \n",fabs((a->getMax()-a->getVal())/a->getErrorHi()));
	fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tErrLo: %f", a->getErrorLo());
        fprintf(CloseCoutSentry::trueStdOutGlobal(), ",\tMin: %f \n",a->getMin());
        fprintf(CloseCoutSentry::trueStdOutGlobal(), "\tPullMin: %f \n",fabs((a->getVal()-a->getMin())/(-1.0*a->getErrorHi())));*/
	/*"\tValue: ", a->getVal()
	"\tErrHi: "<<a->getErrorHi()<<",\tMax: "<<a->getMax()<<
	"\tPullMax: "<<fabs((a->getMax()-a->getVal())/a->getErrorHi())<<
	"\tErrLo: "<<a->getErrorLo()<<",\tMin: "<<a->getMin()<<
	"\tPullMin: "<<fabs((a->getVal()-a->getMin())/(-1.*a->getErrorLo()))<< endl;*/
    }	
