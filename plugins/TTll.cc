...
class TTll : public HaNaBaseMiniAnalyzer{
public:
  explicit TTll(const edm::ParameterSet&);
  ~TTll();
...
DEFINE_FWK_MODULE(TTll);

void TTll::initHistograms(){   
    ...
    hMETSignificance= new Histograms(SampleName, "METSignificance", 100, 0, 100.);
...
TTll::~TTll() {}
TTll::TTll( const edm::ParameterSet& ps ) :
...
//
void TTll::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  W = 1.0;
  ...
  if( jetReader->selectedBJets.size() > 1 ){
    ...
    double metSig = metReader->ReadMetSig(iEvent);
    hMETSignificance->Fill(metSig,W);
  }
}
